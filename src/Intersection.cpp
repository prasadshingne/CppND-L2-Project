...
void WaitingVehicles::permitEntryToFirstInQueue()
{
    ...
    shared_ptr<Vehicle> veh = _vehicles[0]; // get the entries from the front of _vehicles
    promise<void> promis = move(_promises[0]); // get the entries from the front of _promises

    promis.set_value(); // fulfil promise and send signal back

    _vehicles.erase(_vehicles.begin()); // remove front element
    _promises.erase(_promises.begin()); // remove front element
...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    promise<void> promis;
    future<void> futur = promis.get_future();
    _waitingVehicles.pushBack(vehicle, move(promis));
    futur.wait();
